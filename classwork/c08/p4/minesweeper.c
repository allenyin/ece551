...
board_t * makeBoard(int w, int h, int numMines) {
    //WRITE ME!
    board_t * myBoard = (board_t *) malloc(sizeof(board_t));
    myBoard->width = w;
    myBoard->height = h;
    myBoard->totalMines = numMines;
    myBoard->board = (int **) malloc(sizeof(int *)*h);
    // initialize the row pointers first
    for (int i=0; i<h; i++) {
        myBoard->board[i] = (int *) malloc(sizeof(int)*w);
    }
    // initialize values in all squares
    for (int i=0; i<h; i++) {
        for (int j=0; j<w; j++) {
            myBoard->board[i][j] = UNKNOWN;
        }
    }
    // now we initialize mines
...
int inBound(int x, int y, int right_edge, int bot_edge) {
    // check if (x,y) is within the board
    return ((x >= 0) && (x <= right_edge) && \
            (y >= 0) && (y <= bot_edge));
}

int countMines(board_t * b, int x, int y) {
    // count number of mines in an 8-square around (x,y)
    int right_edge = b->width-1;
    int bot_edge = b->height-1;
    int mine_count = 0;

    mine_count += (inBound(x-1, y-1, right_edge, bot_edge) && \
            IS_MINE(b->board[y-1][x-1]));
    mine_count += (inBound(x, y-1, right_edge, bot_edge) && \
            IS_MINE(b->board[y-1][x]));
    mine_count += (inBound(x+1, y-1, right_edge, bot_edge) && \
            IS_MINE(b->board[y-1][x+1]));

    mine_count += (inBound(x-1, y, right_edge, bot_edge) && \
            IS_MINE(b->board[y][x-1]));
    mine_count += (inBound(x+1, y, right_edge, bot_edge) && \
            IS_MINE(b->board[y][x+1]));
